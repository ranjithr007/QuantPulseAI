"""add concurrent indexes for PnL, risk, and time-series evidence queries

Revision ID: pg_20260902_pnl_indexes
Revises: pg_20260831_strategy_indexes
"""
...
revision = "pg_20260902_pnl_indexes"
down_revision = "pg_20260831_strategy_indexes"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    for table_name, name, columns in INDEXES:
        if table_name not in tables or name in _index_names(bind, table_name):
            continue
...
def downgrade():
    ...
    for table_name, name, _columns in reversed(INDEXES):
        if table_name not in tables or name not in _index_names(bind, table_name):
            continue
        with op.get_context().autocommit_block():
            op.drop_index(
                name,
