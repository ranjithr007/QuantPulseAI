...
class Phase1FrontendDashboardStaticTests(unittest.TestCase):
    ...
    def test_pnl_page_shows_the_account_wide_trade_ledger_without_row_caps(self):
        ...
        self.assertIn("export async function loadOpenPaperPositions", dashboard_api)
        self.assertIn("export async function loadPaperTradeHistory", dashboard_api)
        self.assertIn("export async function loadPaperAccountSummary", dashboard_api)
        self.assertIn('"/paper-trade/open-positions"', dashboard_api)
        self.assertIn('"/paper-trade/trade-history"', dashboard_api)
        self.assertIn('"/paper-trade/account-summary"', dashboard_api)
        ...
        self.assertIn("loadPaperTradeMeasurement", dashboard_data)
        self.assertIn("operational_exit_quality", dashboard_data)
        self.assertIn("Historical PNL contains operationally delayed exits", pnl_section)
        self.assertIn("operationally contaminated", pnl_section)
        self.assertIn("Exit-data quality unavailable", pnl_section)
        self.assertIn("loadPaperTradeHistory({", history)
        ...
        self.assertIn('label="Profit factor"', pnl_section)
        self.assertIn('label="Payoff ratio"', pnl_section)
        self.assertIn("performanceHealth={performanceHealth}", main)
        self.assertIn("averageProfit={averageProfit}", main)
        self.assertIn("pnlBreakdownScope={pnlBreakdownScope}", main)
        self.assertIn("pnlCohorts={pnlCohorts}", main)
        self.assertIn("headlineMeasurement={headlineMeasurement}", main)
        self.assertIn("cleanPerformance={cleanPerformance}", main)
        self.assertIn("confidenceCalibration={confidenceCalibration}", main)
        self.assertIn("returnDecomposition={returnDecomposition}", main)
        self.assertIn("measurementEvaluation={measurementEvaluation}", main)
        self.assertIn("measurementDataQuality={measurementDataQuality}", main)
        self.assertIn("exitClassificationCohorts={exitClassificationCohorts}", main)
        self.assertIn("Headline versus clean strategy evidence", pnl_section)
        self.assertIn("Confidence calibration evidence", pnl_section)
        self.assertIn("Return and trading-cost decomposition", pnl_section)
        self.assertIn("Slippage is already embedded", pnl_section)
        self.assertIn("Confidence cannot justify promotion or larger sizing", pnl_section)
        self.assertIn("PNL by exact exit classification", pnl_section)
        self.assertIn("clean strategy evidence remains negative", pnl_section)
        self.assertIn("lack an explicit trailing-activation value", pnl_section)
        self.assertIn("Full-cohort PNL drivers", pnl_section)
        self.assertIn("breakdown.by_strategy", pnl_section)
        self.assertIn("breakdown.by_regime", pnl_section)
        self.assertIn("breakdown.by_timeframe", pnl_section)
        self.assertIn("breakdown.by_exit_reason", pnl_section)
        ...
        self.assertIn("paperTradeOpenPositions", dashboard_data)
        self.assertIn("ledgerScope.symbol_filter !== null", pnl_section)
        self.assertIn("Loading the account-wide wallet summary", pnl_section)
        self.assertIn("trade history load independently", pnl_section)
    ...
    def test_auxiliary_pollers_pause_hidden_pages_and_do_not_overlap(self):
        ...
        self.assertIn("paperReport?.evidence_overall", validation_badge)
        self.assertIn("Clean closed:", validation_badge)

        strategies = (
            FRONTEND_ROOT / "src" / "pages" / "StrategiesPage.jsx"
        ).read_text(encoding="utf-8")
        self.assertIn("Clean official-gate sample", strategies)
        self.assertIn("deadline-breached exit(s) excluded", strategies)
        self.assertIn("CLEAN_EVIDENCE_RECALCULATED_PREVIEW", strategies)
        self.assertIn("cannot promote a strategy", strategies)
        self.assertIn("Timely exit evidence", strategies)
        self.assertIn("missing recorded exit evidence", strategies)
        self.assertIn("five-second promotion-quality limit", strategies)
        self.assertIn("Official recovery readiness", strategies)
        self.assertIn("Automatic resume disabled", strategies)
        self.assertIn("strategyPagination.visibleRows.map", strategies)
        self.assertIn('ariaLabel="Governed paper strategies pagination"', strategies)
    ...
    def test_heavy_page_work_is_progressive_and_worker_managed(self):
        ...
        self.assertIn('if (activePage === "pnl")', dashboard_data)
        self.assertIn("loadOpenPaperPositions({ signal: controller.signal })", dashboard_data)
        self.assertIn("loadPaperTradeHistory({ page: 1, limit: 10, signal: controller.signal })", dashboard_data)
        self.assertIn("loadPaperTradePerformance({ signal: controller.signal })", dashboard_data)
        self.assertIn("loadPaperAccountSummary({ signal: controller.signal })", dashboard_data)
        self.assertIn("Promise.allSettled(corePnlRequests)", dashboard_data)
