        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-slate-400">
              Versioned signals, blockers, executions and net results are measured independently. The symbol lock remains shared across every strategy.
            </p>
          </div>
          <button
            type="button"
    ...
    <article className="overflow-hidden rounded-xl border border-white/10 bg-slate-900/70">
      ...
      <div className="p-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          ...
          <Metric label="Trades" value={performance.total_trades || 0} icon={Layers3} />
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <ValueCard label="Net P&L" value={`₹${number(performance.net_pnl_inr, 2)}`} tone={performance.net_pnl_inr >= 0 ? "emerald" : "rose"} />
          <ValueCard label="Account return" value={formatSigned(performance.account_return_percent || 0, 2) + "%"} tone={performance.account_return_percent >= 0 ? "emerald" : "rose"} />
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-xl border border-white/10">
      <div className="flex items-center justify-between border-b border-white/10 bg-slate-950/60 px-4 py-3">
        ...
        <div className="text-xs text-slate-500">Signal → confirmation → plan → paper position</div>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-[1080px] w-full text-left text-xs">
          <thead className="bg-slate-950/40 text-[10px] uppercase tracking-[0.14em] text-slate-500">
            <tr><th className="px-4 py-2.5">Coin</th><th>Side / TF</th><th>Score</th><th>Confidence</th><th>Strategy decision</th><th>Market Move</th><th>Lifecycle</th><th>Reason</th><th>Evaluated IST</th></tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            ...
            {!candidates.length ? <tr><td colSpan="9" className="px-4 py-8 text-center text-slate-500">Waiting for the next strategy scan.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>
